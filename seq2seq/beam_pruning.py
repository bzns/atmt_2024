...
class BeamSearch(object):
    ...
    def __init__(self, beam_size, max_len, pad):
        ...
        self.best_final_score = float('-inf')  # Track the best score among final hypotheses
    ...
    def add_final(self, score, node):
        ...
        self.best_final_score = max(self.best_final_score, -score)  # Update best final score
    ...
    def prune(self):
        """ Prune unfinished nodes that have lower scores than the best completed hypothesis. """
        nodes = PriorityQueue()
        while not self.nodes.empty():
            score, _, node = self.nodes.get()
            # Only retain nodes with scores higher than the best final score
