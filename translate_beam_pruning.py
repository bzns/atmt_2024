...
def main(args):
    ...
    for i, sample in enumerate(progress_bar):
        ...
        for _ in range(args.max_len-1):
            ...
            max_seq_length = max(len(node.sequence) for node in nodes)
            prev_words = torch.stack([
                F.pad(torch.tensor(node.sequence), (0, max_seq_length - len(node.sequence)), value=tgt_dict.pad_idx)
                for node in nodes
            ])
        ...
        # Find the maximum sequence length among best sequences
        max_seq_length = max(len(search.get_best()[1].sequence[1:]) for search in searches)

        # Pad all sequences to the maximum length
        best_sents = torch.stack([
            F.pad(search.get_best()[1].sequence[1:].cpu(), (0, max_seq_length - len(search.get_best()[1].sequence[1:])), value=tgt_dict.pad_idx)
            for search in searches
        ])
